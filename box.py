...
def isvalid(roll, remaining):
    ...
    for i in range(1, len(remaining)+1):
        #this is basically a list of values of True/False (true if sum of tup combo equals roll; combo comes from itertools combinations)
        if any([sum(combo) == roll for combo in combinations(remaining, i)]): #any(iterable) returns True if any item in iterable is true
            return True
    return False
